...
class StockController {
    ...
    try {
      ...
      if (!product_id || !warehouse_id || !quantity || !adjustment_type) {
        return res.status(400).json({
          success: false,
          error:
            "product_id, warehouse_id, quantity, and adjustment_type are required",
        });
      }
    ...
    try {
      ...
      if (!product_id || !from_warehouse_id || !to_warehouse_id || !quantity) {
        return res.status(400).json({
          success: false,
          error:
            "product_id, from_warehouse_id, to_warehouse_id, and quantity are required",
        });
      }
  ...
  async getHistory(req, res, next) {
    try {
      ...
      const history = await Stock.getStockHistory(
        product_id,
        warehouse_id,
        limit,
      );
